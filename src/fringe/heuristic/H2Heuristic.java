...
/**
 * A heuristic that defines distance to a goal by the sum of the Manhattan
 * distances from each tile to its position in the goal.
 * @author Neil Hulbert
 * @version 1.0
 */
public class H2Heuristic implements Heuristic {
    /**
        ...
        for (int i = 0; i < goalInds.length; i++) {
            ...
            for (int h = 0; h < Grid.NUM_TILES; h++) {
                ...
                if (tile != 0) { // 0 is the blank tile
                    ...
                    accum += Math.abs(goalLoc[0] - loc[0])
                           + Math.abs(goalLoc[1] - loc[1]);
                }
            }
    ...
    public List<Integer> deltaCompute(final Grid grid,
        ...
        for (int i = 0; i < goalInds.length; i++) {
            ...
            out.add(prevHScores.get(i) + curManhattanDist - prevManhattanDist);
        }
